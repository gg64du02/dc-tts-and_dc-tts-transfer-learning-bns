...
def remove_sil(path_in, path_out, format="wav"):
    ...
    # non_sil_times = detect_nonsilent(sound, min_silence_len=10, silence_thresh=sound.dBFS * 3.5)
    #on the first 155 samples
    #10.14
    #non_sil_times = detect_nonsilent(sound, min_silence_len=10, silence_thresh=sound.dBFS * 3.75)
    #10.10
    #non_sil_times = detect_nonsilent(sound, min_silence_len=10, silence_thresh=sound.dBFS * 3.675)
    non_sil_times = detect_nonsilent(sound, min_silence_len=10, silence_thresh=sound.dBFS * 3.5625)
    ...
    if len(non_sil_times) > 0:
        ...
        if len(non_sil_times) > 1:
            for t in non_sil_times[1:]:
                if t[0] - non_sil_times_concat[-1][-1] < 20:
